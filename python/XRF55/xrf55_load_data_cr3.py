...
class XRF55_BaseDataset_cr3(Dataset):
    ...
    def _load_data(self):
        ...
        if self.split_type == "cr2":
            user_range = list(range(1, 31)) if self.train else [31, 32, 33]
        elif self.split_type == "cr3":
            user_range = list(range(1, 31)) if self.train else [34, 35, 36]
        elif self.split_type == "cr4":
            user_range = list(range(1, 31)) if self.train else [37, 38, 39]
        else:

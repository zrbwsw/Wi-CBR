...
class F1Calculator:
    ...
    def __init__(self, num_classes: int = 8, class_names: List[str] = None):
        ...
        # Default class names for 8 gesture actions
        default_class_names = [
            'Drawing circle', 'Drawing cross', 'Pushing', 'Pulling',
            'Swiping left', 'Swiping right', 'Swiping up', 'Swiping down'
        ]
        self.class_names = class_names or (default_class_names if num_classes == 8 else [f'Class_{i}' for i in range(num_classes)])
        self.reset()

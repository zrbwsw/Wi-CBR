...
% ========== Key modifications ==========
% Directly read current receiver file
try
    [csi_data, ~] = csi_get_all(filename);
catch err
    error('File read failed: %s (%s)', filename, err.message);
end

% Initialize output matrix (single receiver 3D structure)
...
% Spectrum truncation and normalization

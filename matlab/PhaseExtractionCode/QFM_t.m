% CSI 可视化：可视化每个 TR 对的数据，首先打开 untitled1.fig，然后运行此代码
% 设置原始 CSI 数据和输出图像的文件路径
filepath = 'D:\Widar3.0\CSI\20181205\user3\'; % 原始 CSI 数据路径
sapath = 'D:\Widar3.0\QFM\FME\'; % 保存处理后图像的路径
        ...
        for on = 1:5 % 第三参数（face orientation）的循环
            for rn = 1:5 % 第四参数（repetition number）的循环
                for rsn = 1:6 % 第五参数（Wi-Fi receiver id）的循环
                    mfm = zeros(1, 3); % 初始化数组，存储每个天线的均值与方差比率
                    % 构建当前数据集的文件名
                    ...
                    package = zeros(90, 128); % 初始化包以存储 CSI 值

                    % 从数据中提取 CSI 跟踪
                    csi_trace = c1(1:dl, 1);
                    num_segments = floor(dl / 128); % 计算分割段数                    
                    for segment = 1:num_segments
                        qfm = zeros(30, 128); % 初始化每段128个数据的相位矩阵
                        k = 128 * (segment - 1); % 偏移索引
                       
                        for j2 = 1:3 % 遍历三个天线
                            for i = 1:128 % 每128个 CSI 条目处理一次
                                row = 0; % 包中的行索引
                                csi_entry = csi_trace{i + k}; % 获取当前 CSI 条目                                
                                csi = get_scaled_csi(csi_entry); % 对 CSI 条目进行缩放
                                j1 = 1; % 因为只有一个发射器
                                for j3 = 1:Num_subcarrier % 遍历每个子载波
                                    row = row + 1; % 行索引递增
                                    % 将 CSI 值存储到包中
                                    package((j2 - 1) * 30 + row, i) = csi(j1, j2, j3);
                                end
                            end
                        ...
                        % 构建保存图像的文件名
                        sname = [suname, '-', num2str(mn), '-', num2str(ln), '-', num2str(on), '-', num2str(rn), '-r', num2str(rsn), '_seg', num2str(segment)];
